...
from lib.utils.renderer_riding1 import Renderer
...
def main(args):
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        for frame_idx in tqdm(range(len(image_file_names))):
            ...
            filename=img_fname.split("/")[-1]
            df = pd.read_csv("./sitting/detection-results/"+filename+'.txt', sep=" ", header=None, names=["class", "prob", "x1","y1","x2","y2"])
            for index, row in df.iterrows():
                cate,x1,y1,x2,y2 = row["class"], row["x1"], row["y1"], row["x2"], row["y2"]
                data_list = df.loc[df['class'].str.contains('chair')]
                # print(data_list)
                if cate=="chair":
                    chair_box.append(np.array([x1,y1,x2,y2]))
                    break
                elif len(data_list)==0:
                    chair_box.append(np.array([x1,y1,x2,y2]))
                    break
        # print (chair_box)
        chair_box=np.array(chair_box)
        chair_box=chair_box[:172]
        # print (chair_box.shape)
        # print ((pred_cam.shape))
        # use the fixes cam pose to fix the chair
        chair_cam= np.tile(pred_cam[50],(pred_cam.shape[0],1))
        ...
        vibe_results_objects[person_id] = output_dict
    ...
    joblib.dump(vibe_results_objects, os.path.join(output_path, "vibe_output_objects.pkl"))
    ...
    if not args.no_render:
        ...
        frame_results_objects = prepare_rendering_results(vibe_results_objects, num_frames)
        mesh_color = {k: colorsys.hsv_to_rgb(np.random.rand(), 0.5, 1.0) for k in vibe_results_objects.keys()}
        ...
        for frame_idx in tqdm(range(len(image_file_names))):
            ...
            frame_cam_1st = None
            # print (len(frame_results_objects))
            # print (len(frame_results))
                # frame_cam_1st = frame_results[1][person_id]['cam']
            for object_id,object_data in frame_results_objects[10].items():
                frame_cam_1st = object_data['cam']
                # print (">>>>>>>>>>>>>>>>>><<<>>>>>>>>>>>>>>>>>>>")
                # print (frame_results[frame_idx])
                for (object_id,object_data), (person_id, person_data) in zip(frame_results_objects[frame_idx].items(),frame_results[frame_idx].items()):
                    # print (">>>>>>>>>>>>>>???>>>>>>>>>>>>>>>>>>>>>>>")
                    # print (frame_results[person_id].keys())
                    frame_cam_1st=object_data['cam']
                ...
                # print (os.path.join(output_img_folder, f'{frame_idx:06d}.png'))
                ...
                if args.display:
                    ...
                    if cv2.waitKey(1) & 0xFF == ord('q'):
                        break
            # for person_id, person_data in frame_results_objects[1].items():
            #     frame_cam_1st = person_data['cam']
            #     for person_id, person_data in frame_results_objects[frame_idx].items():
            #         # print ('....................................>>>>>>>>>>>>>>>>>>>>>')
            #         # print (frame_results[person_id].keys())
            #         frame_verts = person_data['verts']
            #         frame_cam = person_data['cam']
