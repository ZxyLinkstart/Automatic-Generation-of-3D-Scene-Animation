...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        # y = torch.tensor(6, dtype=int)[None]
        ...
        # output_60 = self.seqTransDecoder(tgt=timequerie_60, memory=z[None],
        #                                     tgt_key_padding_mask=~mask_60)
        # prior_vec = output_60
        # output = output_60

        # y = torch.tensor(4, dtype=int)[None]
        # z =  batch["z"]+ self.actionBiases[y]
        #  # sequence of size 1
        ...
        # timequerie_last60  = torch.concat((timequerie_60,timequerie_last60),dim=0)

        # # # timequeries_120 = torch.concat(output_60,timequerie_last60)
        # timequerie_last60 = self.sequence_pos_encoder(timequerie_last60)[:60,:,:]
        # output_last60 = self.seqTransDecoder(tgt=timequerie_last60, memory=z[None],
        #                                     tgt_key_padding_mask=~mask_60)
        # # # # z_prior = self.mlp( z)
        ...
        #     else:
        #         action.append(6)
        # y = torch.tensor(action[0], dtype=int)[None]
        # z0 = z + self.actionBiases[y]
        # z0 = z0[None]  # sequence of size 1
        #   # 生成60帧
        # mask_1 = mask[:,0:1]
        # timequerie_1 =  torch.zeros(1, bs, latent_dim, device=z.device)
        # timequerie_60 = self.sequence_pos_encoder(timequerie_1)
        # output_1 = self.seqTransDecoder(tgt=timequerie_60, memory=z0,
        #                                     tgt_key_padding_mask=~mask_1)
        # prior_vec = output_1
        # output = output_1

        # z_pro = z
        # for i in range(0,nframes):
        #     y = torch.tensor(action[i], dtype=int)[None]
        #     z =  z + self.actionBiases[y]
        #     # z_prior = prior_vec*0.2+z[None]
            
        #     # z_prior = torch.concat((prior_vec,z[None]),dim=0)
        #     # z_prior = self.mlp( z_prior)
        #     #  单帧生成 test
        ...
        #     else:
        #         mask_one =  mask[:,i-1: i+1]

        #     timequerie_one = self.sequence_pos_encoder(timequerie_one)
            
        #     if i > 0:
        #         timequerie_one = torch.concat((timequerie_one,prior_vec),dim=0)

        #     output_one = self.seqTransDecoder(tgt=timequerie_one, memory=z[None],
        #                                     tgt_key_padding_mask=~mask_one)
        #     if i == 0:
        #         output  = output_one
        #         prior_vec = timequerie_one
        #     else:
        #         output = torch.concat((output,output_one[:-1,:,:]),dim=0)
        #         prior_vec = timequerie_one[:-1,:,:]
        ...
        #     #     action.append(6)
        #     # y = torch.tensor(action[i], dtype=int)[None]
        #     z_out =batch['z']+ self.actionBiases[y]

        #     if i>0:
        #         z = torch.concat((z,z_out[None]),dim=0)
        #     else:
        #         z = z_out[None]
        # y_1= torch.tensor(6, dtype=int)[None]
        # y_2= torch.tensor(4, dtype=int)[None]
        # y_3 = torch.tensor(12, dtype=int)[None]
        # z_1 =  self.actionBiases[y_1]
        # z_2 =  self.actionBiases[y_2]
        # z_3 = self.actionBiases[y_3]
        # z = z + self.actionBiases[y_1] * 1.1 + self.actionBiases[y_2] - self.actionBiases[y_3]*1.1
        # z = z + self.actionBiases[y_1]
        # z =z+ z + self.actionBiases[y_1]*1.5 + self.actionBiases[y_2] 
        # z = z_1 * 1.3 + z_2 -z_3
        # z = z_1+ z_2  - z_3*1.2
        # z = z_2
        # z  =( 3*z +z_1 * 1.1  + z_2 - z_3*1.3)/3
        # z = 2*z + z_1*1.5 +  z_2 - z_3
        # y= torch.tensor(0, dtype=int)[None]
        z = z + self.actionBiases[y]
        timequeries = torch.zeros(nframes, bs, latent_dim, device=z.device)
        timequeries = self.sequence_pos_encoder(timequeries)
        # timequerie_60 =  torch.zeros(60, bs, latent_dim, device=z.device)
        # timequerie_60 = self.sequence_pos_encoder(timequerie_60)
        output = self.seqTransDecoder(tgt=timequeries, memory=z[None],
