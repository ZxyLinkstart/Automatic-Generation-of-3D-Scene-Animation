...
class CAE(nn.Module):
    ...
    def generate_one(self, cls, duration, fact=1, xyz=False):
        ...
        mask = self.lengths_to_mask(lengths[None])
    ...
    def generate_ours(self, classes, durations, nspa=1,
        ...
        if flag == 1:
            nats = 5
            y = torch.tensor([0,1,2,3,4], dtype=int, device=self.device).repeat(nspa)
            lengths = torch.tensor(60, dtype=int, device=self.device).repeat(nats)
            mask = self.lengths_to_mask(lengths.repeat(nspa))
            ...
            class_type =11
            y = torch.tensor(class_type, dtype=int, device=self.device).repeat(nspa)
            lengths = torch.tensor(60, dtype=int, device=self.device)
            mask = self.lengths_to_mask(lengths.repeat(nspa))
        ...
        # joblib.dump( batch["output_xyz"],"/data1/zxy/ACTOR_O/meta/{}_mesh.pkl".format(class_type))
        return batch


    def generate(self, classes, durations, nspa=10,
                 noise_same_action="random", noise_diff_action="random",
                 fact=1):
        if nspa is None:
            nspa = 1
        ...
        if noise_same_action == "random":
            ...
            if noise_diff_action == "random":
                ...
                raise NotImplementedError("Noisdie ff action must be random or same.")
