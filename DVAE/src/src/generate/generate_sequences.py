...
def generate_actions(beta, model, dataset, epoch, params, folder, num_frames=240,
    ...
    with torch.no_grad():
        if not onlygen:
            ...
            generation = model.generate_ours(classes, gendurations, nspa=nspa,
            ...
            output = generation["output_xyz"].reshape(nspa, *generation["output_xyz"].shape[1:]).cpu().numpy()
...
def main():
    ...
    if parameters["mode"] in []:   # ["gen", "duration", "interpolate"]:
        ...
        dataset = datasets["test"]  # same for ntu
